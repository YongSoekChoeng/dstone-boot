...
# 🐳 Dstone-Boot Docker 빌드 및 실행 가이드

---

## 1. 컨테이너 구성 개요

이 가이드는 Docker Compose를 사용하여 `dstone-boot` 애플리케이션 및 관련 인프라 서비스를 컨테이너로 구성하는 방법을 설명합니다.

**구성 컨테이너:**

1.  **`dstone-mysql`**: 데이터베이스 (MySQL)
2.  **`dstone-rabbitmq`**: 메시지 큐 (RabbitMQ)
3.  **`dstone-redis`**: 인메모리 캐시 및 데이터 저장소 (Redis)
4.  **`dstone-boot`**: 메인 웹 애플리케이션 프레임워크

> 모든 컨테이너는 **통합 실행** 또는 **개별 실행**이 가능합니다.
...
2.  **템플릿 파일 복사**
    -   `dstone-boot` 관련 설정 파일과 `docker-compose` 파일을 아래 구조에 맞게 `/workshop` 디렉토리로 복사합니다.
    ...
    /workshop
      ...
      └─ dstone-boot-docker-compose.yml  # 통합 실행용 메인 Compose 파일
    ```

3.  **실행 파일 준비**
    > ⚠️ **중요:** `target` 디렉토리는 `.gitignore`에 의해 버전 관리에서 제외됩니다. 따라서 Git에서 소스를 받은 후, `dstone-boot` 프로젝트를 빌드하여 생성된 `*.war` 파일을 `/workshop/dstone-boot/target/` 디렉토리에 수동으로 복사해야 합니다.

4.  **파일 권한 설정 및 형식 변환 (Linux/WSL)**
    -   스크립트 실행 및 파일 접근 권한을 설정합니다.
    ```bash
    cd /workshop
    sudo chown -R root:root *
    sudo chmod -R 777 *
    # Windows에서 작성된 파일의 경우, 개행 문자를 변환합니다.
...
`dstone-boot-docker-compose.yml` 파일을 사용하여 모든 서비스를 한 번에 실행합니다.
...
-   **서비스 중지 및 컨테이너/볼륨 모두 삭제**
    > ⚠️ **주의:** `-v` 옵션은 `dstone-mysql`의 데이터 볼륨을 포함한 모든 Docker 볼륨을 영구적으로 삭제합니다. 데이터베이스 정보를 유지하려면 이 옵션을 사용하지 마세요.
    ```bash
...
각 서비스의 `01.*-docker.yml` 파일을 사용하여 개별적으로 컨테이너를 실행합니다. 서비스 단위의 테스트 및 개발에 유용합니다.

### 4.1. 실행 순서 및 주의사항

> ⚠️ **실행 순서:** `dstone-boot` 애플리케이션은 다른 서비스에 의존하므로, **반드시 `dstone-mysql`, `dstone-redis`, `dstone-rabbitmq`를 먼저 실행해야 합니다.** 이 세 서비스 간의 실행 순서는 무관합니다.
...
필요에 따라 각 서비스의 이미지를 빌드하여 Docker Hub에 푸시할 수 있습니다. 각 서비스 디렉토리의 `02.*-docker-reg.sh` 스크립트를 사용하세요.
